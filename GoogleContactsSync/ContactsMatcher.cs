		...
		public static void SyncContact(ContactMatch match, Syncronizer sync)
		{
            ...
            try
            {
                ...
                else if (match.OutlookContact == null && match.GoogleContact != null)
                {
                    ...
                    outlookContactItem = OutlookConnection.Application.CreateItem(Outlook.OlItemType.olContactItem) as Outlook.ContactItem;
                ...
                else if (match.OutlookContact != null && match.GoogleContact != null)
                {
                    ...
                    if (lastSynced.HasValue)
                    {
                            ...
                            && lastUpdatedGoogle.Subtract(lastSynced.Value).TotalSeconds > TimeTolerance)
                        {
                            ...
                            {
                                case SyncOption.Merge:
                                    if (lastUpdatedGoogle > lastUpdatedOutlook)
                                    {
                                        Logger.Log("Google item will overwrite Outlook one because it's fresher one: " + match.OutlookContact.FileAs, EventType.Information);
                                        sync.UpdateContact(match.GoogleContact, outlookContactItem);
                                    }
                                    else if (lastUpdatedGoogle < lastUpdatedOutlook)
                                    {
                                        Logger.Log("Outlook item will overwrite Google one because it's fresher one: " + match.OutlookContact.FileAs, EventType.Information);
                                        sync.UpdateContact(outlookContactItem, match.GoogleContact);
                                    }
                                    else
                                        Logger.Log("Both items were updated at the same time. Can't define which version prevails", EventType.Warning);
                                    break;
                    ...
                    else
                    {
                        ...
                        {
                            case SyncOption.Merge:
                                //lastUpdated... is stored without seconds. take that into account.
                                DateTime lastUpdatedOutlook = match.OutlookContact.LastModificationTime.AddSeconds(-match.OutlookContact.LastModificationTime.Second);
                                DateTime lastUpdatedGoogle = match.GoogleContact.Updated.AddSeconds(-match.GoogleContact.Updated.Second);
                                if (lastUpdatedGoogle > lastUpdatedOutlook)
                                {
