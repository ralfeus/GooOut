...
namespace R.GoogleOutlookSync
{
	internal class Syncronizer
	{
        ...
        internal SyncResult Result { get { return this._result; } }
		...
		public void LoginToGoogle(string username, string password)
		{
            ...
            {
                var rs = new RequestSettings(Application.ProductName, username, password);
                if (_syncContacts)
                    _contactsRequest = new ContactsRequest(rs);
                //if (_syncNotes)
                //    _documentsRequest = new DocumentsRequest(rs);
            }
            Google.GData.Calendar.CalendarService service = new Google.GData.Calendar.CalendarService(Application.ProductName);
            service.setUserCredentials(username, password);
            this._googleAuthToken = service.QueryClientLoginToken();
			...
			lock (_syncRoot)
			{
                ...
                try
                {
                    ...
                    foreach (var sync in this._syncBatch)
                        this._result += sync.Sync();

                    Logger.Log(String.Format("Synchronization is finished\r\n{0}", this._result), EventType.Information);
                    this._result.Succeeded = true;
                }
